import { Dimensions, Image, SafeAreaView, StyleSheet, Text, TextInput, ToastAndroid, TouchableOpacity, View } from 'react-native'
...
const SignlePost = ({ route, navigation }) => {
    const dispatch = useDispatch();
    const { postId } = route.params;
    const { loading, user, diamonds } = useSelector((state) => state.auth);
    const { loading: postLoading, post: item, IsDeleted, message: msg } = useSelector((state) => state.post);
        ...
        loading || postLoading ? <Loading /> :
            <SafeAreaView style={{ flex: 1, backgroundColor: Colors.white }} forceInset={{ top: 'always' }}>
                <View style={[{ flex: 1, height: Dimensions.get('window').height - 53 }]}>
                    ...
                    <View style={{ position: 'absolute', zIndex: 1, bottom: 0, padding: 15 }}>
